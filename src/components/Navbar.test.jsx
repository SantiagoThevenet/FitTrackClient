...
describe("Navbar", () => {
  ...
  it("should test the children inside my NavBar component when user is authenticated", () => {
    ...
    // Realiza tus comprobaciones para verificar el comportamiento cuando isAuthenticated es true
    expect(navBar.children[0].props.href).toBe("/");
    console.log(navBar.children[1].children[2].children[0])
    expect(navBar.children[1].children[2].children[0]).toBe(
      "LOGOUT"
    );
    expect(navBar.children[1].children[1].children[0].props.href).toBe(
      "/routines"
